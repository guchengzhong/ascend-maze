...
def _overall_figure(results: Mapping[str, Mapping[str, object]], path: Path) -> None:
    canvas = SvgCanvas(1080, 430, "Overall performance comparison")
    _figure_title(
        canvas,
        "Overall performance",
        "Fixed Batch=20 | 20/20 succeeded on both executors | bars normalized per metric",
    )
    panels = (
        (
            "E2E P95",
        ...
        (
            ...
            lambda value: f"{value:.4f} req/s",
            "higher",
        ),
    )
    panel_lefts = (42.0, 381.0, 720.0)
    panel_width = 318.0
    for separator_x in (360.0, 699.0):
        canvas.line(separator_x, 101, separator_x, 392, stroke="#e4e8ec")
    for left, (title, direction, values, formatter, preference) in zip(
        panel_lefts, panels, strict=True
    ):
        canvas.text(left, 118, title, size=17, weight=700)
        canvas.text(left, 140, direction, size=10, fill=COLORS["muted"], weight=600)
        maze_value = values.get("maze", 0.0)
        ray_value = values.get("ray", 0.0)
        if preference == "lower":
            winner = "maze" if maze_value <= ray_value else "ray"
            winner_value = min(maze_value, ray_value)
            loser_value = max(maze_value, ray_value)
            difference = (
                (loser_value - winner_value) / loser_value * 100
                if loser_value > 0
                else 0.0
            )
            comparison = f"{winner.capitalize()} {difference:.1f}% lower"
        else:
            winner = "maze" if maze_value >= ray_value else "ray"
            winner_value = max(maze_value, ray_value)
            loser_value = min(maze_value, ray_value)
            ratio = winner_value / loser_value if loser_value > 0 else 0.0
            comparison = f"{winner.capitalize()} {ratio:.2f}x higher"
        canvas.text(left, 181, comparison, size=21, fill=COLORS[winner], weight=700)
        canvas.text(left, 201, "relative result", size=10, fill=COLORS["muted"])

        maximum = max(values.values(), default=1.0) or 1.0
        track_left = left + 47
        track_width = 155
        for row_index, executor in enumerate(("maze", "ray")):
            y = 252 + row_index * 58
            value = values.get(executor, 0.0)
            canvas.text(left, y + 4, executor.capitalize(), size=11, weight=600)
            canvas.rect(
                track_left,
                y - 7,
                track_width,
        ...
        canvas.line(left, 354, left + panel_width, 354, stroke="#e4e8ec")
        canvas.text(
            left,
            378,
            "Same 20 fixed requests",
            size=10,
            fill=COLORS["muted"],
        )
    canvas.save(path)
...
def _paired_speedup_figure(
    ...
    row_height = 25
    top = 116
    plot_bottom = top + row_height * len(pairs)
    canvas_height = max(300, plot_bottom + 44)
    canvas = SvgCanvas(1080, canvas_height, "Paired request speedup")
    median_ratio = statistics.median([2**item[0] for item in pairs]) if pairs else 1.0
    _figure_title(
        canvas,
        "Paired request speedup",
        f"{len(pairs)} matched requests | median {median_ratio:.2f}x "
        "Ray/Maze | log2 scale",
    )
    left = 285
    right = 1000
    minimum_score = min((item[0] for item in pairs), default=-0.5)
    maximum_score = max((item[0] for item in pairs), default=1.0)
    axis_min = min(-0.5, math.floor(minimum_score * 2) / 2)
    axis_max = max(1.0, math.ceil(maximum_score * 2) / 2)

    def x_for(score: float) -> float:
        return left + (score - axis_min) / (axis_max - axis_min) * (right - left)
    ...
    canvas.text((left + center) / 2, 83, "Ray faster", size=11, anchor="middle", weight=600)
    canvas.text(
        (center + right) / 2,
        83,
        "Maze faster",
    ...
    for half_step in range(round(axis_min * 2), round(axis_max * 2) + 1):
        exponent = half_step / 2
        x = x_for(exponent)
        is_parity = half_step == 0
        canvas.line(
            x,
            top - 9,
            x,
            plot_bottom + 9,
            stroke=COLORS["axis"] if is_parity else COLORS["grid"],
            stroke_width=2 if is_parity else 1,
        )
        canvas.text(
            x,
            102,
            "1x" if is_parity else f"{2**exponent:.2g}x",
            size=10,
            anchor="middle",
            weight=700 if is_parity else 400,
        )
    for index, (score, maze, ray) in enumerate(pairs):
        ...
        label = (
            f"{maze.get('dataset')}.{maze.get('workflow')}  "
            f"#{maze.get('request_index')}"
        )
        canvas.text(left - 11, y + 4, label, size=10, anchor="end")
        x = x_for(score)
        family = str(maze.get("family"))
        color = COLORS.get(family, COLORS["muted"])
        canvas.rect(
            min(center, x),
            y - 5,
            abs(x - center),
        ...
        canvas.circle(x, y, 4, fill=color)
        ratio = 2**score
        canvas.text(
            x + (8 if score >= 0 else -8),
            y + 4,
            f"{ratio:.2f}x",
            size=10,
    ...
    legend_y = canvas_height - 15
    canvas.rect(827, legend_y - 9, 13, 9, fill=COLORS["text"], rx=2)
    canvas.text(847, legend_y, "Text", size=11)
    canvas.rect(913, legend_y - 9, 13, 9, fill=COLORS["vision"], rx=2)
    canvas.text(933, legend_y, "Vision", size=11)
    canvas.save(path)
